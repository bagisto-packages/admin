<template>
    <div class="control-group" :class="[errors.has(name) ? 'has-error' : '']">
        ...
        <select v-if="this.options.length" v-validate="checkValidations" class="control" :id="name" :name="name"
                v-model="savedValue"
                :data-vv-as="label">
            <option v-for='(option, index) in this.options' :value="option.value"> {{ option.title }}</option>
        </select>

        <input v-else type="text" class="control" v-validate="checkValidations" :id="name" :name="name"
               v-model="savedValue"
               :data-vv-as="label">
