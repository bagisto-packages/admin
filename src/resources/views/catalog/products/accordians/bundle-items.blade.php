        ...
        <accordian :active="true">
            ...
            <div slot="body">
                <div class="control-group" :class="[errors.has(titleInputName + '[label]') ? 'has-error' : '']">
                    ...
                    <input type="text" v-validate="'required'" :name="titleInputName + '[label]'" v-model="option.label"
                           class="control"
                           data-vv-as="&quot;{{ __('admin::app.catalog.products.option-title') }}&quot;"/>
                ...
                <div class="control-group" :class="[errors.has(inputName + '[type]') ? 'has-error' : '']">
                    ...
                    <select v-validate="'required'" :name="inputName + '[type]'" v-model="option.type" class="control"
                            data-vv-as="&quot;{{ __('admin::app.catalog.products.input-type') }}&quot;">
                ...
                <div class="control-group" :class="[errors.has(inputName + '[is_required]') ? 'has-error' : '']">
                    ...
                    <select v-validate="'required'" :name="inputName + '[is_required]'" v-model="option.is_required"
                            class="control"
                            data-vv-as="&quot;{{ __('admin::app.catalog.products.is-required') }}&quot;">
                ...
                <div class="control-group" :class="[errors.has(inputName + '[sort_order]') ? 'has-error' : '']">
                    ...
                    <input type="text" v-validate="'required|numeric|min_value:0'" :name="inputName + '[sort_order]'"
                           v-model="option.sort_order" class="control"
        ...
        <div>
            <div class="control-group">
                ...
                <input type="text" class="control"
                       placeholder="{{ __('admin::app.catalog.products.product-search-hint') }}"
                       v-model.lazy="search_term" v-debounce="500" autocomplete="off">

                <div class="linked-product-search-result">
                    <ul>
                        <li v-for='(product, index) in searched_results' v-if='searched_results.length'
        ...
        <tr>
            <td>
                <div class="control-group" v-if="bundleOption.type == 'radio' || bundleOption.type == 'select'">
                    <span class="radio">
                        <input type="radio" :name="[inputName + '[is_default]']" :id="[inputName + '[is_default]']"
                               :value="product.is_default" @click="checkProduct($event)" :checked="product.is_default">
            ...
            <td>
                <div class="control-group" :class="[errors.has(inputName + '[qty]') ? 'has-error' : '']">
                    <input type="number" v-validate="'required|min_value:1'" :name="[inputName + '[qty]']"
                           v-model="product.qty" class="control"
            ...
            <td>
                <div class="control-group" :class="[errors.has(inputName + '[sort_order]') ? 'has-error' : '']">
                    <input type="number" v-validate="'required|min_value:1'" :name="[inputName + '[sort_order]']"
                           v-model="product.sort_order" class="control"
                           data-vv-as="&quot;{{ __('admin::app.catalog.products.sort-order') }}&quot;"/>
                    <span class="control-error" v-if="errors.has(inputName + '[sort_order]')">@{{ errors.first(inputName + '[sort_order]') }}</span>
                </div>
            </td>
