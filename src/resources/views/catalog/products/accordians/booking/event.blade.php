        ...
        <div>
            <div class="section">
                <div class="secton-title">
                    <span>{{ __('admin::app.admin.catalog.products.tickets') }}</span>
                </div>
    ...
    <script type="text/x-template" id="ticket-list-template">
        <div class="ticket-list table">
            ...
            <button type="button" class="btn btn-lg btn-primary" style="margin-top: 20px" @click="addTicket()">
                {{ __('admin::app.admin.catalog.products.add-ticket') }}
            </button>
        </div>
    </script>
        ...
        <tr>
            <td>
                <div class="control-group" :class="[errors.has(controlName + '[{{$locale}}][name]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.name') }}</label>
                    <input type="text" v-validate="'required'" :name="controlName + '[{{$locale}}][name]'" v-model="ticketItem.name" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.name') }}&quot;">
                ...
                <div class="control-group" :class="[errors.has(controlName + '[special_price]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.special-price') }}</label>
                    <input type="text" v-validate="{decimal: true, min_value:0, ...(ticketItem.price ? {max_value: ticketItem.price} : {})}" :name="controlName + '[special_price]'" v-model="ticketItem.special_price" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.price') }}&quot;">
            ...
            <td>
                <div class="control-group" :class="[errors.has(controlName + '[price]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.price') }}</label>
                    <input type="text" v-validate="'required|decimal|min_value:0'" :name="controlName + '[price]'" v-model="ticketItem.price" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.price') }}&quot;">
            ...
            <td>
                <div class="control-group" :class="[errors.has(controlName + '[qty]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.quantity') }}</label>
                    <input type="text" v-validate="'required|min_value:0'" :name="controlName + '[qty]'" v-model="ticketItem.qty" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.qty') }}&quot;">
                ...
                <div class="control-group date" :class="[errors.has(controlName + '[special_price_to]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.special-price-to') }}</label>

                    <datetime>
                        <input type="text" v-validate="'date_format:yyyy-MM-dd HH:mm:ss|after:special_price_from'" :name="controlName + '[special_price_to]'" v-model="ticketItem.special_price_to" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.special-price-to') }}&quot;" ref="special_price_to" style="width:100%"/>
                    </datetime>
            ...
            <td>
                <div class="control-group" :class="[errors.has(controlName + '[{{$locale}}][description]') ? 'has-error' : '']">
                    <label class="ticket-label">{{ __('admin::app.admin.catalog.products.description') }}</label>
                    <textarea type="text" v-validate="'required'" :name="controlName + '[{{$locale}}][description]'" v-model="ticketItem.description" class="control" data-vv-as="&quot;{{ __('admin::app.admin.catalog.products.description') }}&quot;"></textarea>
